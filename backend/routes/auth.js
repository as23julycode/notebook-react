...
router.post("/createuser", [
  ...
  async (req, res) => {
    ...
    if (!error.isEmpty()) {
      return res.status(500).json({ success, errors: error.array() });
    }
    ...
    try {
      ...
      if (user) {
        return res.status(400).json({success, error: "user with email Already registered" });
      }
...
router.post("/login", [
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!passwordCompare) {
        success = false;
        return res.status(400).json({ success, errors: "Plese try to login with correct credential" });
      }
      const data = {
        user: {
          id: user.id
